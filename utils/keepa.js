...
// Keepa's stats.min / stats.max are the ALL-TIME low / high for each CSV type --
// i.e. Keepa's own "Buy Box - Lowest" and "Buy Box - Highest". Each entry is a
// [timestamp, value] pair (prices in cents), or -1 when there is no data -- NOT
// the flat scalars stats.current holds. (The 90-day interval extremes live in
// minInInterval / maxInInterval; we deliberately use the all-time fields here.)
export const statExtreme = (stats, kind, idx) => {
  const entry = stats?.[kind === "max" ? "max" : "min"]?.[idx];
